...
| ファイル | 用途 | サイズ |
|---|---|---|
| `icon.png` | iOS アプリアイコン | 1024×1024（透過なし） |
...
| ストア | 必要なサイズ |
|---|---|
| App Store | iPhone 6.9"（1320×2868）と 6.5"（1242×2688）。iPad 対応を謳うなら 13" も |
| Google Play | スマホ用 2枚以上 + フィーチャーグラフィック 1024×500 |

撮り方: 実機かシミュレータで撮影し、`app.json` の `supportsTablet` を
`false` にすれば iPad 用スクリーンショットの提出義務がなくなる。
